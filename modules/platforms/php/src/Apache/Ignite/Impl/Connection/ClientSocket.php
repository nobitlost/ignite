...
use Apache\Ignite\Impl\Binary\Request as Request;
use Apache\Ignite\Exception\IgniteClientException as IgniteClientException;
use Apache\Ignite\Exception\OperationException as OperationException;
...
class ClientSocket {
    ...
    public function connect(): void {
        ...
        if (!$this->socket = stream_socket_client(
            ...
            throw new IgniteClientException($errstr, $errno);
        }
        // send handshake
    ...
    private function processRequest(Request $request): void {
        ...
        while (($length = strlen($buffer)) > 0) {
            ...
            if ($written === false) {
                throw new IgniteClientException('Error while writing data to the server');
            }
    ...
    private function receive(MessageBuffer $buffer, int $minSize): void {
        while ($buffer->getLength() < $minSize) 
        {
            ...
            if ($chunk === false || $chunk === '') {
                throw new IgniteClientException('Error while reading data from the server');
            }
            else {
                $buffer->append($chunk);
            }
    ...
    private function processResponse(Request $request): void {
        ...
        $this->receive($buffer, BinaryUtils::getSize(TypeInfo::TYPE_INTEGER));
        // Response length
        $length = $buffer->readInteger();
        $this->receive($buffer, $length + BinaryUtils::getSize(TypeInfo::TYPE_INTEGER));
        if ($request->isHandshake()) {
            // Handshake status
            $isSuccess = ($buffer->readByte() === ClientSocket::HANDSHAKE_SUCCESS_STATUS_CODE);
            $requestId = $request->getId();
            $this->finalizeHandshake($buffer, $isSuccess);
        }
        else {
            // Request id
            ...
            $this->finalizeResponse($buffer, $request, $isSuccess);
        }
        $this->logMessage($requestId, false, $buffer->getBuffer());
    }
    
    private function finalizeHandshake(MessageBuffer $buffer, bool $isSuccess): void {
        if (!$isSuccess) {
            ...
            if (!$this->protocolVersion->equals($serverVersion)) {
                if (!$this->isSupportedVersions($serverVersion) ||
                    ...
                    $handshakeRequest = $this->getHandshake($serverVersion);
                    $this->sendRequest($handshakeRequest);
                }
            }
            else {
                $this->disconnect();
    ...
    private function finalizeResponse(MessageBuffer $buffer, Request $request, bool $isSuccess): void {
        if (!$isSuccess) {
            // Error message
            $errMessage = BinaryReader::readObject($buffer);
            throw new OperationException($errMessage);
